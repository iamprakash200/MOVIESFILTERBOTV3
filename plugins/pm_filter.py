...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("𝚃𝙷𝙸𝚂 𝙸𝚂 𝙽𝙾𝚃 𝙵𝙾𝚁 𝚈𝙾𝚄, 𝚁𝙴𝚀𝚄𝙴𝚂𝚃 𝙰 𝙽𝙴𝚆 𝙾𝙽𝙴 𝙵𝙾𝚁 𝚈𝙾𝚄", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('📣 𝚄𝙿𝙳𝙰𝚃𝙴𝚂 🎯'', url='https://t.me/vkmoviesrequesr_zone')
            ],[
